...
def wait_for_bitcoind_start(process, url, i):
    '''
    Wait for xnz to start. This means that RPC is accessible and fully initialized.
    Raise an exception if xnz exits during initialization.
    '''
    while True:
        if process.poll() is not None:
            raise Exception('xnz exited with status %i during initialization' % process.returncode)
        try:
...
def initialize_chain(test_dir):
    ...
    if (not os.path.isdir(os.path.join("cache","node0"))
        ...
        # Create cache directories, run xnzs:
        for i in range(4):
            datadir=initialize_datadir("cache", i)
            args = [ os.getenv("XNZ", "xnz"), "-server", "-keypool=1", "-datadir="+datadir, "-discover=0" ]
            if i > 0:
                args.append("-connect=127.0.0.1:"+str(p2p_port(0)))
            bitcoind_processes[i] = subprocess.Popen(args)
            if os.getenv("PYTHON_DEBUG", ""):
                print "initialize_chain: xnz started, waiting for RPC to come up"
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None):
    """
    Start a xnz and return RPC connection to it
    """
    datadir = os.path.join(dirname, "node"+str(i))
    if binary is None:
        binary = os.getenv("XNZ", "xnz")
    ...
    bitcoind_processes[i] = subprocess.Popen(args)
    if os.getenv("PYTHON_DEBUG", ""):
        print "start_node: xnz started, waiting for RPC to come up"
...
def start_nodes(num_nodes, dirname, extra_args=None, rpchost=None, binary=None):
    """
    Start multiple xnzs, return RPC connections to them
    """
