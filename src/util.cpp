...
const char * const BITCOIN_CONF_FILENAME = "xnz.conf";
const char * const BITCOIN_PID_FILENAME = "xnzd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "xnz" is a composite category enabling all Xnz-related debug output
            if(ptrCategory->count(string("xnz"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\XnzCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\XnzCore
    // Mac: ~/Library/Application Support/XnzCore
    // Unix: ~/.xnzcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "XnzCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/XnzCore";
#else
    // Unix
    return pathRet / ".xnzcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty xnz.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override xnz.conf
