...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "XnzNetwork") type = XnzNetwork;
    if(strType == "XnzNetworkVariable") type = XnzNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == XnzNetwork) s = "XnzNetwork";
    if(type == XnzNetworkVariable) s = "XnzNetworkVariable";
